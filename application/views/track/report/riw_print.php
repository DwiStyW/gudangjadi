...
<table border="1" width="100%">
    <thead>
        ...
        <?php
$in = $this->db->query("SELECT SUM(masuk) AS salIn FROM riwayattrack WHERE kode='$kode' && tglform between '0001-01-01' AND '$mulai'");
$out = $this->db->query("SELECT SUM(keluar) AS salOut FROM riwayattrack WHERE kode='$kode' && tglform between '0001-01-01' AND '$mulai'");
?>
        <tr>
...
$riw = $this->db->get('master');
foreach ($riw->result() as $m) {
    $sals = $dataIn - $dataOut;
    $sat1 = floor($sals / ($m->max1 * $m->max2));
    $sisa = $sals - ($sat1 * $m->max1 * $m->max2);
    $sat2 = floor($sisa / $m->max2);
    $sat3 = $sisa - $sat2 * $m->max2;
}?>
    </thead>
    <tbody>
        <tr>
...
$satn1 = floor($r->keluar / ($r->max1 * $r->max2));
    ...
    ?>
        ...
        <?php }
        $allin = $this->db->query("SELECT SUM(masuk) AS sumIn FROM riwayattrack WHERE  keluar='0'&& kode='$kode' && tglform between '$start' AND '$end'");
        $allout = $this->db->query("SELECT SUM(keluar) AS sumOut FROM riwayattrack WHERE masuk='0' && kode='$kode' && tglform between '$start' AND '$end'");
